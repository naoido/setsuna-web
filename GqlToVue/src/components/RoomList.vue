...
import { GET_ROOMS, ROOM_ADDED, POST_REGISTER, GET_USER } from '../graphql/queries'
...
onMounted(() => {
   /*ユーザー情報を取得
  const { result: userResult, loading: userLoading, error: userError } = useQuery(GET_USER)

  if (userError.value) {
    console.error('GraphQL Error:', userError.value)
  }

  watch(userResult, (newUserResult) => {
    if (newUserResult) {
      user.value = newUserResult.user
